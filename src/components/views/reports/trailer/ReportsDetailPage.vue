<template>
  ...
  <div v-else  class="section__page trucks">
    <DetailNav>
      <DeatilNavBack :title="trailer_history_inspect_id?.trailer?.number || '---'"  span="Trailers Number:" path="/reports"/>
    </DetailNav>
...
import {
  getTrailerHistoryInspect,
  trailer_history_inspect_id,
  updateTrailerHistoryById
} from "@/hooks/trailer/useTrailerHistory";
import InfoTrailerTruck from "@/components/views/reports/InfoTrailerTruck";
import {updateTruckHistoryById} from "@/hooks/truck/useTruckHistory";
export default {
  components: {
    InfoTrailerTruck,
    TrailersDrawItem,
  ...
  setup(){
    ...
    async function fetchData(){
      await getTrailerHistoryInspect(router_id);
      if(trailer_history_inspect_id.value?.trailer?.assigned_driver?.id){
        await getFormListEntities({entities: 'need_repair_devices', limit: 99});
        if(trailer_history_inspect_id.value) {
          data_target.value = Object.assign({}, trailer_history_inspect_id.value);
          data_target.value.trailer_id = trailer_history_inspect_id.value.trailer.id;
          data_target.value.driver_id = trailer_history_inspect_id.value.trailer.assigned_driver.id;
          data_target.value.location = trailer_history_inspect_id.value?.location || '';
    ...
    function save(){
      ...
      if(validate.value) {
        updateTrailerHistoryById({id:router_id,data:data_target.value});
        comment.value = '';
      } else {
        toast('400','error')
      }
    ...
    return{
      data_target,
      form_list_entities,
      validate, trailer_history_inspect_id,
      modalPhoto,
      image_draw,
      data1,
      data2,
